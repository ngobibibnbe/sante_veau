...
for index, visit in  visits.iterrows():
    if visit["Pneumonie"]==True and visit["Diarrhé"]==False and 300<= visit["Duration"]<= 600:
        ...
        new_path= pneumony_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])+"/"
        check_path(video_path)
        #shutil.copyfile( visit["names"] , video_path+str(visit['names']).split("vealnum")[1] )
        shutil.move(video_path+str(visit['names']).split("vealnum")[1] , new_path+str(visit['names']).split("vealnum")[1])


    elif visit["Pneumonie"]==False and visit["Pneumonie"]==True and 300<= visit["Duration"]<= 600:
        video_path= diarrhé_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])
        new_path= diarrhé_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])+"/"
        check_path(video_path)
        #shutil.copyfile( visit["names"] , video_path+str(visit['names']).split("vealnum")[1] )
        shutil.move(video_path+str(visit['names']).split("vealnum")[1] , new_path+str(visit['names']).split("vealnum")[1])

    elif visit["Pneumonie"]==True and visit["Pneumonie"]==True and 300<= visit["Duration"]<= 600: 
        video_path= pneumony_and_diarrhéa_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])
        new_path= pneumony_and_diarrhéa_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])+"/"
        check_path(video_path)
        #shutil.copyfile( visit["names"] , video_path+str(visit['names']).split("vealnum")[1] )
        shutil.move(video_path+str(visit['names']).split("vealnum")[1] , new_path+str(visit['names']).split("vealnum")[1])

    elif visit["Pneumonie"]==False and visit["Pneumonie"]==False and 300<= visit["Duration"]<= 600 :
        video_path= health_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])
        new_path= health_path+'/'+str(visit["station"])+'/'+str(visit["calfNumber"])+"/"
        check_path(video_path)
        #shutil.copyfile( visit["names"] , video_path+str(visit['names']).split("vealnum")[1] )
        shutil.move(video_path+str(visit['names']).split("vealnum")[1] , new_path+str(visit['names']).split("vealnum")[1])
